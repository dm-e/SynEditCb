...
void __fastcall TCommandsDataModule::DataModuleCreate(TObject* Sender)
{
	...
	SynCppSyn1->DefaultFilter = "C++ files (*.cpp,*.h,*.hpp)|*.cpp;*.h;*.hpp";
	...
	SynSQLSyn1->DefaultFilter = "SQL files (*.sql)|*.sql";
...
void __fastcall TCommandsDataModule::DataModuleDestroy(TObject* Sender)
{
	...
	//RemoveComponent(SynCppSyn1);
	TComponent *Temp;
	for (int I = ComponentCount - 1; I >= 0; I--)
	{
		Temp = Components[I];
		if (dynamic_cast<TSynCustomHighlighter*>(Temp) != NULL) {
