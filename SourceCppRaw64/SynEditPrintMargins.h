...
public:
 ...
 // Right position of text in device units (pixels) - calculated form
   ...
   // Top position of text in device units (pixels) - this is the top
	...
	void __fastcall InitPage(TCanvas* ACanvas, int PageNum, Syneditprinterinfo::TSynEditPrinterInfo* PrinterInfo, bool LineNumbers, bool LineNumbersInMargin, int MaxLineNum);
	virtual void __fastcall Assign(TPersistent* Source);
	void __fastcall LoadFromStream(TStream* AStream);
	void __fastcall SaveToStream(TStream* AStream);
__published:
public:
	__property Syneditprinttypes::TUnitSystem UnitSystem = { read = FUnitSystem, write = FUnitSystem, default = /*# usMM */ 0 };
