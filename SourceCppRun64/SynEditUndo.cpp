...
void __fastcall TSynEditUndoList::EnsureMaxEntries()
{
	...
	if(Count > FOwner->fMaxUndoActions)
	{
		...
		KeepCount = ((int)(FOwner->fMaxUndoActions / 4)) * 3;
		ItemArray = ToArray();
		...
		for(stop = (int) ItemArray.Length - 1, i = ItemArray.Length - KeepCount; i <= stop; i++)
		{
...
void __fastcall TSynCaretAndSelectionUndoItem::Redo(TCustomControl* xEditor)
{
	TCustomSynEdit* Editor = dynamic_cast<TCustomSynEdit*>(xEditor);
	// Same as Undo
	((TCustomSynEdit*)Editor)->SetCaretAndSelection(FCaret, fBlockBegin, fBlockEnd);
}

void __fastcall TSynCaretAndSelectionUndoItem::Undo(TCustomControl* xEditor)
{
	TCustomSynEdit* Editor = dynamic_cast<TCustomSynEdit*>(xEditor);
...
__fastcall TSynLinesDeletedUndoItem::TSynLinesDeletedUndoItem(TCustomSynEdit* Editor, int Index, TArray<String>& DeletedLines)
 : FIndex(Index),
			FLines(DeletedLines)
{
	//# inherited::Create();
...
void __fastcall TSynLinesDeletedUndoItem::Redo(TCustomControl* xEditor)
{
	TCustomSynEdit* Editor = dynamic_cast<TCustomSynEdit*>(xEditor);
	((TSynEditStringList*) Editor->Lines)->DeleteLines(FIndex, (int) FLines.Length);
	FCaret = BufferCoord(1, FIndex + 1);
}

void __fastcall TSynLinesDeletedUndoItem::Undo(TCustomControl* xEditor)
{
	TCustomSynEdit* Editor = dynamic_cast<TCustomSynEdit*>(xEditor);
	((TSynEditStringList*) Editor->Lines)->InsertStrings(FIndex, FLines);
	FCaret = BufferCoord(1, Min((int)Editor->Lines->Count, (int) (FIndex + FLines.Length + 1)));
}
...
void __fastcall TSynLinesInsertedUndoItem::Redo(TCustomControl* xEditor)
{
	TCustomSynEdit* Editor = dynamic_cast<TCustomSynEdit*>(xEditor);
	((TSynEditStringList*) Editor->Lines)->InsertStrings(FIndex, FLines);
	FCaret = BufferCoord(1, Min(Editor->Lines->Count, (int)(FIndex + FLines.Length + 1)));
}

void __fastcall TSynLinesInsertedUndoItem::Undo(TCustomControl* xEditor)
{
	TCustomSynEdit* Editor = dynamic_cast<TCustomSynEdit*>(xEditor);
	((TSynEditStringList*) Editor->Lines)->DeleteLines(FIndex, (int) FLines.Length);
	FCaret = BufferCoord(1, FIndex + 1);
...
void __fastcall TSynLinePutUndoItem::Redo(TCustomControl* xEditor)
{
	TCustomSynEdit* Editor = dynamic_cast<TCustomSynEdit*>(xEditor);
	String Line;
	int Char = 0;
...
void __fastcall TSynLinePutUndoItem::Undo(TCustomControl* xEditor)
{
	TCustomSynEdit* Editor = dynamic_cast<TCustomSynEdit*>(xEditor);
	String Line;
	int Char = 0;
