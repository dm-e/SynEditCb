...
{
	throw ESynEditStringList(SListIndexOutOfBounds, ARRAYOFCONST((Index)));
}

__fastcall TSynEditStringList::TSynEditStringList(const TExpandAtWideGlyphsFunc& AExpandAtWideGlyphsFunc)
 : FList(nullptr),
			FCount(0),
			FCapacity(0),
			FFileFormat(sffDos),
...
void __fastcall TSynEditStringList::Delete(int Index)
{
	...
	if(Index < FCount)
	{
		System::Move(&(*FList)[Index + 1], &(*FList)[Index], static_cast<size_t>((FCount - Index) * SynEditStringRecSize));
	}
	FIndexOfLongestLine = -1;
	if(Assigned(FOnDeleted))
...
void __fastcall TSynEditStringList::DeleteLines(int Index, int NumLines)
{
	...
	if(NumLines > 0)
	{
		...
		try
		{
			...
			if(LinesAfter > 0)
				System::Move(&(*FList)[Index + NumLines], &(*FList)[Index], static_cast<size_t>(LinesAfter * SynEditStringRecSize));
			FCount -= NumLines;
			if(Assigned(FOnDeleted))
...
String __fastcall TSynEditStringList::GetSeparatedText(String Separators)
{
  ...
  // handle 1st line separately (to avoid trailing line break)
	...
	if(L != 0)
	{
		System::Move(ustr2address(PRec->FString), P, static_cast<size_t>(L * sizeof(Char)));
		P += L;
	}
	++PRec;
	for(stop = FCount - 1, I = 1; I <= stop; I++)
	{
		switch(LineBreakSize)
		{
			...
			break;
			default:
			System::Move(PLineBreak, P, static_cast<size_t>(LineBreakSize * sizeof(Char)));
			P += LineBreakSize;
			break;
		}
...
void __fastcall TSynEditStringList::InsertItem(int Index, const String S)
{
	...
	if(Index < FCount)
	{
		System::Move(&(*FList)[Index], &(*FList)[Index + 1], static_cast<size_t>((FCount - Index) * SynEditStringRecSize));
	}
	FIndexOfLongestLine = -1;
	/*# with FList^[Index] do */
	{
...
void __fastcall TSynEditStringList::InsertStrings(int Index, D2CArray<String>& Strings, int FromIndex/*# = 0*/)
{
	int I = 0;
	int LineCount = 0;
	if((Index < 0) || (Index > FCount))
	...
	if(LineCount > 0)
	{
		...
		try
		{
			...
			if(Index < FCount)
			{
				System::Move(&(*FList)[Index], &(*FList)[Index + LineCount], static_cast<size_t>((FCount - Index) * SynEditStringRecSize));
			}
...
void __fastcall TSynEditStringList::SetCapacity(int NewCapacity)
{
	if(NewCapacity < Count)
		new /*#possible memory leak*/ EListError(SInvalidCapacity);
...
//		TSynEditStringFlags Flags = (*FList)[Index].FFlags;
//
//		Flags = Flags - Synedittextbuffer__6;
//
//		if(Value.Contains(sfModified))
//			Flags << sfModified;
//
//		if(Value.Contains(sfSaved))
//			Flags << sfSaved;
//
//		if(Value.Contains(sfAsSaved))
//			Flags << sfAsSaved;
//
//		(*FList)[Index].FFlags = Flags;
		(*FList)[Index].FFlags = (*FList)[Index].FFlags - Synedittextbuffer__6 + ConvertSubset<TSynEditStringFlags>(Value);
	}
}
