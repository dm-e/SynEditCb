...
TDisplayCoord __fastcall TCustomSynEdit::PixelsToNearestRowColumn(int AX, int AY)
{
  ...
  // don't return a partially visible last line
	...
	result.Column = (int) Max((double) 1, LeftChar + Round(f));
...
void __fastcall TCustomSynEdit::WMDropFiles(::TMessage& Msg)
{
	...
	try
	{
		if(ASSIGNED(fOnDropFiles))
		{
			...
			{
				int stop = 0;
				iNumberDropped = (int) DragQueryFile(((THandle) Msg.WParam), ((unsigned int) -1), nullptr, 0);
				DragQueryPoint(((THandle) Msg.WParam), &Point);
				for(stop = iNumberDropped - 1, i = 0; i <= stop; i++)
				{
					DragQueryFileW(((THandle) Msg.WParam), (UINT) i, FileNameW, (unsigned int)(sizeof(FileNameW) / /*div*/ 2));
...
{
	Msg.Result = StrLen(StrLCopy(Msg.Text, ustr2pwchar(Text), Msg.TextMax - 1));
}
	...
	if((Msg.LParam & GCS_RESULTSTR) != 0)
	{
      ...
      // ImeCount is always the size in bytes, also for Unicode
			...
			try
			{
				...
				PW[(int)(ImeCount / /*div*/ sizeof(WideChar))] = L'\x00';
