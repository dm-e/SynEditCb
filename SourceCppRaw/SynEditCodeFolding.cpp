...
void __fastcall TSynFoldRanges::AddByParts(int AFoldType, int AFromLine, int AToLine/*# = -1*/)
{
  ...
  // Insert keeping the list sorted
	FR = TSynFoldRange(AFromLine, AToLine, AFoldType);
...
int __fastcall TSynFoldRanges::GetIndentLevel(int Line)
{
	...
	int stop = 0;
	result = -1;
	fFoldInfoList->BinarySearch(TLineFoldInfo(Line), Index);
  // Search above Line
...
{
	int Index = 0;
	if(fFoldInfoList->BinarySearch(TLineFoldInfo(ALine), Index))
