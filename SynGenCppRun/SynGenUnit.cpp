...
String __fastcall StuffString(const String Value)
{
	...
	{
		if(Value[i] == L'\'')
			result = result + L"''";     // create Delphi string
		else
...
void __fastcall TFrmMain::MakeHashTable()
{
	...
	for(stop = L'\xff', I = L'\x00'; I <= stop; I++)
	{
		WideChar ch = static_cast<WideChar>(I);
        WideChar up = Sysutils::AnsiUpperCase(String(ch))[1];

		switch(CharInSet(ch, Syngenunit__2))
		{
			case true:
			mKeyHashTable[I] = int(up) - 64;
			break;
			default:
			mKeyHashTable[I] = 0;
			break;
...
void __fastcall TFrmMain::BtnStartClick(TObject* Sender)
{
	...
	TStringList* LexFileLines;
	ClearAll();
	Screen->Cursor = crHourGlass;
	//LexFileLines = new TUnicodeStringList();
	LexFileLines = new TStringList();
	try
	{
...
bool __fastcall TFrmMain::KeywordsAreAllAlphaNumAndDifferent()
{
	...
	KeyWordList = new TStringList();
	{
		...
		KeyWordList->Duplicates = System::Types::dupError;
		try
		{
			int stop = 0;
			for(stop = KeyList->Count - 1, i = 0; i <= stop; i++)
